defmodule BeamToEx do
  ...
  def convert(options) do
    mod_beam = case Keyword.has_key?(options, :erl) do
        true -> {:ok, mod_beam} = :epp.parse_file(String.to_char_list(options[:erl]), [])
                mod_beam
        false ->  {:ok,{_,[{:abstract_code,{_,mod_beam}}]}} = :beam_lib.chunks(String.to_char_list(options[:beam]), [:abstract_code])
                    mod_beam
    end
